...
public class WAMGUI extends Application{
	...
	this.local = new moleHole[board.getRows()][board.getColumns()];
	this.stage = stage;
	...
	for (int i = 0; i < board.getRows(); i++) {
		...
		moleHole temp = new moleHole(board.getColumns() * i + j, client);
		local[i][j] = temp;
	...
	public moleHole(int pos, WAM.client.WAMClient client){
	    this.client = client;
	    this.pos = pos;
	    this.state =false;
	    ...
	    this.setOnAction(new EventHandler<ActionEvent>() {
			...
			client.moleWhacked(((moleHole) event.getSource()).getPos());
		    }
		});
